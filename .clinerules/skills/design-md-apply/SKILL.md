...
- Component properties can accept **token references** (e.g., `{colors.primary}`), **raw hex color values** (e.g., `#1A1C1E` — must be `#RGB` or `#RRGGBB` format), or **composite references** (e.g., `{typography.body-md}`). Color values must start with `#`; other formats such as `rgba()` or `hsl()` are not valid per the DESIGN.md spec and will not resolve to a `ResolvedColor`.  
...
- **Important**: the `design.md` linter's `contrast-ratio` rule applies a single **4.5:1** minimum to all component pairs regardless of text size. To avoid linter warnings, target 4.5:1 for all text. The 3:1 threshold for large text is a WCAG AA allowance, but the linter does not distinguish text size.  
- If the ratio is below the applicable threshold, add a warning to your output and suggest an alternative token (e.g., a darker background or lighter text from the same palette).  
...
- **Spacing / rounded**: If missing, use sensible agent defaults and inform the user. The spec treats absent `spacing` and `rounded` sections as informational only — they are expected to fall back to agent defaults. Do not block on them, but do not contradict any tokens that are defined.  
  
Never silently apply hardcoded values that conflict with tokens already defined in the design system.  
...
- **Transparent or missing backgrounds**: If a component has no `backgroundColor` defined, or its value cannot be resolved to a hex color, skip contrast validation for that component.  
...
- [ ] Contrast ratios meet WCAG AA (4.5:1 for all text to satisfy the linter; 3:1 minimum for large text per WCAG AA).  
